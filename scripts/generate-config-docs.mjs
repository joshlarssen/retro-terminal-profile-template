...
const presetsPath = path.join(__dirname, 'theme-presets.json');
const outputPath = path.join(rootDir, 'CONFIG.md');

const presets = JSON.parse(await readFile(presetsPath, 'utf8')).presets ?? {};
const teaserGif = profileData.teaserGif ?? {};
const crtEffects = profileData.crtEffects ?? {};

const presetLines = Object.entries(presets)
  .map(([key, value]) => `- \`${key}\` — ${value.label}: ${value.description}`)
  .join('\n');
...
\`\`\`yaml
teaserGif:
  stepDelayMs: ${teaserGif.stepDelayMs ?? 1100}
\`\`\`

- minimum: \`${teaserGif.minStepDelayMs ?? 700} ms\`
- maximum: \`${teaserGif.maxStepDelayMs ?? 2400} ms\`
...
\`\`\`yaml
crtEffects:
  enabled: ${crtEffects.enabled ?? true}
  scanlines: ${crtEffects.scanlines ?? true}
\`\`\`
